...
class ToDo:
    ...
    def update_task(self, task_id, title, description, due_date, task_type, status=None):
        try:


            query = '''UPDATE Task
                       SET Title = %s, Description = %s,  Due_date = %s,status= %s, Task_type = %s
                       WHERE Task_id = %s'''
            status= 'pending'
            params = (title, description, due_date, status, task_type, task_id)
            self.cursor.execute(query, params)
            self.connection.commit()
